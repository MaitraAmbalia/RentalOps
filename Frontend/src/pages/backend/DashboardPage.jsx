...
export default function DashboardPage() {
  ...
  return (
    <div className="space-y-8 font-sans text-text-main max-w-6xl mx-auto">
      
      {/* Header toolbar */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-border-main pb-5">
        <div>
          <h1 className="text-2xl font-extrabold text-text-main tracking-tight">Vendor Workstation Dashboard</h1>
          <p className="text-sm text-text-muted mt-1">Aggregated statistics, fleet performances, and revenue tracking indicators.</p>
        </div>
        ...
        <div className="space-y-8 text-xs">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {statCards.map((card, idx) => {
              ...
              return (
                <div key={idx} className="bg-bg-card border border-border-main rounded-2xl p-5 flex items-center justify-between shadow-sm">
                  <div className="space-y-1">
                    <span className="text-text-muted block uppercase tracking-wider text-[9px] font-bold">{card.name}</span>
                    <span className="text-2xl font-black text-text-main block">{card.value}</span>
                  </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <div className="bg-bg-card p-6 rounded-2xl border border-border-main space-y-4 md:col-span-2">
              ...
              <div className="grid grid-cols-2 gap-4 text-center">
                <div className="bg-bg-main p-4 rounded-xl border border-border-main">
                  <span className="text-[10px] text-text-muted font-bold block uppercase mb-1">Pickups Scheduled</span>
                  <span className="text-2xl font-black text-text-main">{stats.pickupsTodayCount} Tasks</span>
                </div>
                <div className="bg-bg-main p-4 rounded-xl border border-border-main">
                  <span className="text-[10px] text-text-muted font-bold block uppercase mb-1">Returns Due</span>
                  <span className="text-2xl font-black text-text-main">{stats.returnsTodayCount} Tasks</span>
                </div>
              </div>

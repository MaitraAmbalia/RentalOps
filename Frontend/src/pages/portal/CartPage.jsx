...
export default function CartPage() {
  ...
  const handleItemStartDateChange = (item, newStart) => {
    const today = getTodayStr();
    const cleanStart = newStart < today ? today : newStart;
    let newEnd = item.scheduledReturnDate;
    if (cleanStart >= newEnd) {
      newEnd = addDaysStr(cleanStart, 1);
    }
    updateCartItemDates(item.product.id, cleanStart, newEnd, item.variant);
  };

  const handleItemEndDateChange = (item, newEnd) => {
    const minEnd = addDaysStr(item.rentalStartDate, 1);
    const cleanEnd = newEnd <= item.rentalStartDate ? minEnd : newEnd;
    updateCartItemDates(item.product.id, item.rentalStartDate, cleanEnd, item.variant);
  };
    ...
    try {
      for (const item of cart) {
        ...
        const orderPayload = {
          ...
          securityDepositAmount: calculateProductDeposit(item.product, item.product.rentalPrice || item.product.dailyCharge || 0) * item.qty,
        };
  ...
  if (cart.length === 0) {
    return (
      <div className="max-w-md mx-auto text-center py-20 space-y-4">
        <ShoppingBag className="h-16 w-16 text-slate-400 mx-auto" />
        ...
        <Link 
          to="/dashboard" 
          className="inline-block bg-primary hover:bg-primary-hover text-white font-bold text-xs px-5 py-2.5 rounded-xl shadow transition-all"
        >
          Browse Rental Gear
        </Link>
      </div>
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 text-slate-700 font-sans">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              >
                ...
                <div className="space-y-1 sm:col-span-2 text-xs">
                  ...
                  <div className="flex items-center space-x-2 text-[11px] text-slate-500 font-semibold pt-1">
                    ...
                    <span>•</span>
                    <span className="text-primary font-bold">{days} days duration</span>
                  </div>
                  ...
                  <div className="flex space-x-2 pt-2 text-[10px]">
                    <div className="flex items-center space-x-1">
                      ...
                      <input 
                        ...
                        min={getTodayStr()}
                        onChange={(e) => handleItemStartDateChange(item, e.target.value)}
                        className="bg-slate-50 border border-slate-200 rounded p-1 text-[10px] text-slate-700 font-semibold focus:outline-none focus:border-primary cursor-pointer"
                      />
                    </div>
                    <div className="flex items-center space-x-1">
                      ...
                      <input 
                        ...
                        min={addDaysStr(item.rentalStartDate, 1)}
                        onChange={(e) => handleItemEndDateChange(item, e.target.value)}
                        className="bg-slate-50 border border-slate-200 rounded p-1 text-[10px] text-slate-700 font-semibold focus:outline-none focus:border-primary cursor-pointer"
                      />
                    </div>
                  </div>
                ...
                <div className="flex sm:flex-col items-center sm:items-end justify-between gap-3 text-xs">
                  ...
                  <div className="flex items-center space-x-2">
                    <button
                      ...
                      className="w-6 h-6 bg-slate-50 border border-slate-200 font-bold rounded flex items-center justify-center text-xs"
                    >
                      -
                    </button>
        ...
        <div className="bg-white p-6 rounded-3xl border border-slate-200 shadow-sm space-y-6 self-start text-xs text-slate-500">
          ...
          <form onSubmit={handleApplyCoupon} className="space-y-2">
            <label className="block text-[10px] font-bold text-slate-400 uppercase tracking-wider">Coupon Code (use RENT10)</label>
          ...
          <div className="space-y-2 pt-2">
            <button
              ...
              className="w-full py-3 bg-primary hover:bg-primary-hover text-white font-bold rounded-xl text-xs flex items-center justify-center space-x-1.5 shadow-md shadow-primary/10"
            >
      ...
      {checkoutModalOpen && (
        <div className="fixed inset-0 z-50 bg-slate-950/50 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white border border-slate-200 rounded-3xl p-6 w-full max-w-xl space-y-4 shadow-2xl animate-in zoom-in-95 duration-150 overflow-y-auto max-h-[90vh]">
            ...
            <form onSubmit={handleExpressPayment} className="space-y-4 text-xs text-slate-700">
              <div>
              ...
              <div>
                <label className="block font-bold text-slate-500 uppercase tracking-wider mb-1.5">Cardholder Full Name</label>
                <input
                  type="text"
                  name="cardholderName"
                  required
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block font-bold text-slate-500 uppercase tracking-wider mb-1.5">Expiry Date (MM/YY)</label>
                  <input
                    type="text"
                    name="expiryDate"
                    required
                ...
                <div>
                  <label className="block font-bold text-slate-500 uppercase tracking-wider mb-1.5">CVV Code</label>
                  <input
                    type="password"
                    name="cvv"
                    required
              ...
              <div className="border-t border-slate-100 pt-3">
                <label className="block font-bold text-slate-500 uppercase tracking-wider mb-1.5">Billing Street Address</label>
                <input
                  type="text"
                  name="address"
                  required
              ...
              <div className="grid grid-cols-3 gap-3">
                <div>
                  <label className="block font-bold text-slate-500 uppercase tracking-wider mb-1.5">Zip/Postal</label>
                  <input
                    type="text"
                    name="zipCode"
                    required
              ...
              <div className="border-t border-slate-100 pt-4 flex space-x-2">
                ...
                <button
                  ...
                  className="flex-1 py-3 bg-primary hover:bg-primary-hover text-white font-bold rounded-xl flex items-center justify-center space-x-1.5 transition-colors"
                >

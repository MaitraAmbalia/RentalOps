...
import { 
  ArrowLeft, FileText, Send, Check, RefreshCw, X, ShieldAlert, Award, Truck, RotateCcw, DollarSign
} from 'lucide-react';
...
export default function VendorOrderDetailPage() {
  ...
  const handleConfirmQuotation = async () => {
    ...
    try {
      ...
      // 2. Auto-create order using order schema
  ...
  // Calculate refund when penalty inputs change
  const handlePenaltyChange = (days) => {
    ...
    // Cap deduction at securityDepositAmount
    ...
    <div className="space-y-6 max-w-5xl mx-auto">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
                ...
                <tbody className="divide-y divide-slate-900">
                  {record.items?.map((item, idx) => (
                    <tr key={idx}>
                      ...
                      <td className="py-3 text-right text-slate-300">₹{(Number(item.unitPrice || item.product?.rentalPrice) || 0).toFixed(2)}</td>
          ...
          {/* Security Deposit Invoice Card (Sale Orders Only) */}
          {!isQuotation && (
            <div className="bg-slate-950 p-6 rounded-2xl border border-slate-800 space-y-4">
              ...
              {!settled && (
                <div className="flex justify-end pt-2">
                  {settlementOpen ? (
                    <div className="bg-slate-900 p-4 rounded-xl border border-slate-800 w-full space-y-4">
                      <div className="flex items-center justify-between">
        ...
        {/* Right Side: Pricing summary & actions */}
